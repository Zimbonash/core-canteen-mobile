...
export default function LoginScreen({ navigation, onLoginSuccess }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('userRole', roleData.mobile_role);

      // Check if user can access mobile app
      if (roleData.mobile_role === 'web_only') {
        Alert.alert(
          'Web Portal Required',
      ...
      onLoginSuccess(roleData.mobile_role, roleData.user);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Ionicons name="restaurant" size={80} color="#1bb4f4" />
        <Text style={styles.title}>Core Canteen</Text>
